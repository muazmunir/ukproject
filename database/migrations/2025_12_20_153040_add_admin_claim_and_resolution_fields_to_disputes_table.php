...
return new class extends Migration {
  public function up(): void
  {
    Schema::table('disputes', function (Blueprint $table) {
      // Who took/owns this dispute
      $table->foreignId('taken_by_admin_id')->nullable()
        ->after('status')->constrained('users')->nullOnDelete();
      $table->timestamp('taken_at')->nullable()->after('taken_by_admin_id');

      // Who finalized it
      $table->foreignId('resolved_by_admin_id')->nullable()
        ->after('taken_at')->constrained('users')->nullOnDelete();
