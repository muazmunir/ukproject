...
<body style="margin:0;background:#f6f7fb;font-family:Arial,Helvetica,sans-serif;color:#111;">
  @php
    ...
    $servicePrice = ((int)($reservation->subtotal_minor ?? 0)) / 100; // base service price only

    // ✅ Coach commission from service_fees (10% in your screenshot)
    $coachFeePercent = (float) \App\Models\ServiceFee::where('is_active', 1)
        ->where('slug', 'coach_commission')
        ->value('amount') ?? 0;

    $coachFee = round($servicePrice * ($coachFeePercent / 100), 2);
    $coachNet = round($servicePrice - $coachFee, 2);
    ...
    <div style="background:#fff;border-radius:16px;overflow:hidden;box-shadow:0 6px 22px rgba(0,0,0,.06);">
      ...
      <div style="padding:20px 22px;">
        ...
        <div style="color:#555;font-size:13px;margin-bottom:14px;">
          Package: <strong>{{ $packageName }}</strong> · Client: <strong>{{ $clientName }}</strong>
        </div>
        ...
        <div style="border:1px solid #eee;border-radius:14px;padding:14px 14px;margin-bottom:14px;">
          <div style="font-weight:800;margin-bottom:6px;">Scheduled sessions</div>
        ...
        <div style="border:1px solid #eee;border-radius:14px;padding:14px 14px;">
          <div style="font-weight:800;margin-bottom:10px;">Earnings summary</div>
          ...
          <table role="presentation" cellspacing="0" cellpadding="0" style="width:100%;border-collapse:collapse;">
            <tr>
              ...
              <td align="right" style="padding:6px 0;color:#111;font-size:13px;">
                {{ number_format($servicePrice,2) }} {{ $currency }}
              </td>
            </tr>
            <tr>
              <td style="padding:6px 0;color:#444;font-size:13px;">
                Platform fee ({{ rtrim(rtrim(number_format($coachFeePercent,2),'0'),'.') }}%)
              </td>
              <td align="right" style="padding:6px 0;color:#111;font-size:13px;">
                -{{ number_format($coachFee,2) }} {{ $currency }}
              </td>
            </tr>
            <tr>
            ...
            <tr>
              <td style="padding:6px 0;font-weight:800;font-size:14px;">You receive</td>
              <td align="right" style="padding:6px 0;font-weight:800;font-size:14px;">
                {{ number_format($coachNet,2) }} {{ $currency }}
              </td>
            </tr>
          </table>

          <div style="margin-top:10px;color:#666;font-size:12px;line-height:1.45;">
            Note: The client may pay additional customer fees; this summary reflects only your earnings from the service price.
          </div>
        </div>

        <div style="margin-top:16px;color:#666;font-size:12px;line-height:1.45;">
          Please be ready on time. You can manage the booking from your coach dashboard.
        </div>
      </div>
