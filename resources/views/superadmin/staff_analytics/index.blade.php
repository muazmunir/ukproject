...
<div class="sa-page">
    <div class="sa-container">
        ...
        <section class="sa-hero-card">
            <div class="sa-hero-top">
                <div class="sa-hero-copy">
                    ...
                    <h1 class="sa-page-title text-center">Staff Analytics</h1>
                    <p class="sa-page-subtitle text-capitalize text-center">
                        Track support performance, ratings, service length, escalations, and dispute operations
                        with clearer process health metrics and fairer staff contribution visibility.
                    </p>
                </div>
             ...
             <div class="sa-hero-stats">
                    ...
                    <div class="sa-mini-stat">
                        <span class="sa-mini-stat-label">Managers</span>
                        <strong class="sa-mini-stat-value">{{ number_format($summary->total_managers ?? 0) }}</strong>
                    </div>
                    <div class="sa-mini-stat">
        ...
        <div class="sa-tabs-wrap">
            <nav class="sa-tabs">
                @foreach($tabOptions as $key => $label)
                    <a
                        href="{{ route('superadmin.staff_analytics.index', array_merge(request()->query(), ['tab' => $key])) }}"
        ...
        {{-- Filters --}}
        <form method="GET" action="{{ route('superadmin.staff_analytics.index') }}" class="sa-filter-card">
            ...
            <div class="sa-filter-grid">
                ...
                <div class="sa-field">
                    ...
                    <select name="role" class="sa-select">
                        ...
                        <option value="admin" {{ $filters['role'] === 'admin' ? 'selected' : '' }}>Agent</option>
...
@endforeach
            ...
            <div class="sa-filter-actions">
                ...
                <a href="{{ route('superadmin.staff_analytics.index') }}" class="sa-btn sa-btn-secondary">Reset</a>

                @if($selectedStaff)
                    <a href="{{ route('superadmin.staff_analytics.index', array_merge(request()->except('staff_id'), ['tab' => 'overview'])) }}" class="sa-btn sa-btn-light">
        ...
        @if($activeTab === 'overview')
                        ...
                        @for($star = 5; $star >= 1; $star--)
                            <div class="sa-rating-row">
                                <div class="sa-rating-left">
                                    <a href="{{ route('superadmin.staff_analytics.index', array_merge(request()->query(), ['tab' => 'ratings', 'stars' => $star])) }}" class="sa-star-link">
                                        {{ $star }} Star
                                    </a>
                                </div>
            ...
            <section class="sa-grid sa-grid-3">
                ...
                <div class="sa-panel">
                    <div class="sa-panel-head">
                        <div>
                            <h3 class="sa-panel-title ">Service</h3>
                        </div>
                    </div>

                    <div class="sa-list">
                        @forelse($serviceLeaders as $index => $staff)
                            <div class="sa-list-item">
                                ...
                                <div class="sa-list-content">
                                    ...
                                    <div class="sa-list-meta">
                                        <span>{{ $staff->team_name }}</span>
                                        <span>Manager: {{ $staff->team_manager }}</span>
                                        <span>Joined {{ $formatDate($staff->service_start_at) }}</span>
                                        <span>{{ number_format($staff->service_days) }} Days</span>
                                    </div>
                                </div>
                            </div>
                        @empty
                            <div class="sa-empty-state">No service data found.</div>
                        @endforelse
                    </div>
                </div>
            </section>
    ...
    {{ $row->avg_rating !== null ? number_format($row->avg_rating, 2) . ' / 5' : '—' }}
                                    ...
                                    <td>{{ number_format($row->service_days) }} Days</td>
                                </tr>
                            @empty
                                <tr>
                                    <td colspan="13">
            ...
            <section class="sa-grid sa-grid-5">
                @for($star = 5; $star >= 1; $star--)
                    <div class="sa-stat-card">
                        <a
                            href="{{ route('superadmin.staff_analytics.index', array_merge(request()->query(), ['tab' => 'ratings', 'stars' => $star])) }}"
                            class="sa-stat-link">
                        ...
                        <tbody>
                            @forelse($ratings as $row)
                                <tr>
                                    ...
                                    <td>
                                        <a
                                            href="{{ route('superadmin.staff_analytics.index', array_merge(request()->query(), ['tab' => 'ratings', 'stars' => $row->stars])) }}"
                                            class="sa-star-link">
        ...
        @if($activeTab === 'service')
            <section class="sa-grid sa-grid-2">
                <div class="sa-panel">
                    <div class="sa-panel-head">
                        <h3 class="sa-panel-title ">Service Length</h3>
                    </div>
                    <div class="sa-chart-card">
                        <canvas id="serviceLengthChart"></canvas>
                    </div>
                </div>

                <div class="sa-panel">
                    <div class="sa-panel-head">
                        <h3 class="sa-panel-title">Service Leaders</h3>
                    </div>
                    ...
                    <div class="sa-list">
                        ...
                        @empty
                            <div class="sa-empty-state">No service records found.</div>
                        @endforelse
                    </div>
                </div>
            </section>
            ...
            <section class="sa-panel">
                <div class="sa-panel-head">
                    <h3 class="sa-panel-title">Service Table</h3>
                </div>
                        ...
                        <tbody>
                            @forelse($serviceLeaders as $index => $staff)
                                <tr>
                                    ...
                                    <td>
                                        <div class="sa-person-cell">
                                            <a href="{{ $staff->details_url }}" class="sa-name-link">{{ $staff->display_name }}</a>
                                            <span>{{ $staff->email ?: '—' }}</span>
                                        </div>
                                    </td>
                                    <td><span class="sa-badge">{{ $staff->display_role }}</span></td>
                                    <td>{{ $staff->team_name }}</td>
                                    <td>{{ $staff->team_manager }}</td>
                                    <td>{{ $formatDate($staff->service_start_at) }}</td>
                                    <td>{{ number_format($staff->service_days) }} Days</td>
                                </tr>
                            @empty
                                <tr>
                                    <td colspan="7">
        ...
        @if($activeTab === 'staff' || $selectedStaff)
            <section class="sa-panel sa-staff-focus-panel">
                ...
                @else
                    <div class="sa-empty-state text-capitalize">
...
@push('scripts')
    ...
    <script>
        (() => {
            ...
            makeBarChart(
                'serviceLengthChart',
                chartData.service_length_chart.labels,
                chartData.service_length_chart.values,
                'Service Days',
                'rgba(37, 99, 235, 0.75)',
                'rgba(37, 99, 235, 1)'
            );
...
if (teamSelect && staffSelect) {
    const filterStaff = () => {
        ...
        Array.from(staffSelect.options).forEach(option => {
            ...
            if (!selectedTeam) {
                option.style.display = 'block'; // show all
            } else {
                option.style.display = (teamId == selectedTeam) ? 'block' : 'none';
            }
        });

        // reset selected staff if not in team
