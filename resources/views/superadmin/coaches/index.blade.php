...
<label for="seg-deleted">Deleted <span class="badge danger">{{ $counts['deleted'] ?? 0 }}</span></label>
  ...
  <div class="avatar-wrap">
    ...
    <div class="avatar-fallback" style="{{ $avatar ? 'display:none;' : '' }}">
      {{ $letter }}
            ...
            <td>
  @php
    $kyc = (string) ($u->coach_verification_status ?? '');
    $pill = $kyc === 'approved' ? 'ok' : ($kyc === 'pending' ? 'pending' : ($kyc === 'rejected' ? 'danger' : 'pending'));
    $lbl  = $kyc === 'approved' ? 'Approved' : ($kyc === 'pending' ? 'Pending' : ($kyc === 'rejected' ? 'Rejected' : 'Not Submitted'));
  @endphp
  ...
  <div class="row-actions compact">
    {{-- Info / details --}}
    <a href="{{ route('superadmin.coaches.show', $u->id) }}"
   class="btn icon info"
   title="View Details"
...
<a href="{{ route('superadmin.coaches.stats', $u->id) }}"
