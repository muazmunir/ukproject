...
@push('styles')
  <link rel="stylesheet" href="{{ asset('assets/css/client-disputes.css') }}">
  <link rel="stylesheet" href="{{ asset('assets/css/buttons.css') }}">
@endpush

@section('role-content')
@php
  // helper for decision label
...
<div class="dv-page">
  <div class="card">
    <div class="card__head">
      ...
      <a class="btn btn--ghost bg-black text-white rounded-pill" href="{{ route('client.home', ['tab'=>'bookings']) }}">
        Back To Bookings
      </a>
    </div>
      ...
      <table class="dv-table">
        ...
        <tbody>
          @forelse($disputes as $d)
            @php
              $disputeUrl = route('client.disputes.show', $d);
              $bookingUrl = route('client.bookings.show', $d->reservation_id);
...
@else
  @if($statusKey === 'open')
    <span class="dv-metaItem dv-muted">In Queue</span>
  @elseif($statusKey === 'in_review')
    <span class="dv-metaItem dv-muted">In Review</span>
  @else
    <span class="dv-metaItem dv-muted">—</span>
  @endif
@endif

                  {{-- Final decision (only if finalized) --}}
                  @if($isFinal)
                    <span class="dv-dot">•</span>
                    <span class="dv-metaItem">
              ...
              <td class="dv-resCell">
                <a class="dv-resLink text-decoration-none"
                   href="{{ $bookingUrl }}"
                   title="Open Booking"
                   aria-label="Open booking #{{ $d->reservation_id }}">
                  Reservation #{{ $d->reservation_id }}
                  <svg style="width:20px;" class="dv-ext" viewBox="0 0 24 24" aria-hidden="true">
              ...
              <td class="dv-actions">
                <a class="dv-actionBtn"
                  ...
                  <svg viewBox="0 0 24 24" aria-hidden="true">
                    <path d="M4 4h16a2 2 0 0 1 2 2v9a2 2 0 0 1-2 2H9l-4.5 3.4A1 1 0 0 1 3 19.9V6a2 2 0 0 1 2-2zm0 2v11l3.5-2.6c.2-.1.4-.2.6-.2h12a.5.5 0 0 0 .5-.5V6.5a.5.5 0 0 0-.5-.5H4z"></path>
                  </svg>
                </a>
              </td>
            </tr>
...
@push('scripts')
<script>
  // Row click opens dispute (but ignore clicks on links/buttons)
  ...
  // Keyboard: Enter/Space opens dispute row
