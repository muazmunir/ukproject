  ...
  <div class="mx-auto ckyc-max">
    ...
    <div class="card border-0 shadow-sm ckyc-card">
      <div class="card-body ckyc-card-body">
        ...
        <form method="POST" action="{{ route('coach.application.store') }}" enctype="multipart/form-data" novalidate>
          @csrf
          ...
          <div class="mb-3">
            ...
            @error('profile_photo')
              <div class="invalid-feedback d-block ckyc-error">{{ $message }}</div>
            @enderror
          </div>
          ...
          <div class="mb-3">
            ...
            <select name="id_type" id="idType" class="form-select ckyc-select" required>
              ...
              <option value="driving_license" {{ old('id_type') === 'driving_license' ? 'selected' : '' }}>
                {{ __('Driving Licence') }}
              </option>
            </select>
            @error('id_type')
