...
@push('styles')
  <link rel="stylesheet" href="{{ asset('assets/css/coach-dispute-chat.css') }}">
...
@php
  ...
  // coach can chat in open/opened/in_review only if NOT finalized
  ...
  $decidedBy      = $dispute->decidedBy ?? null;
  ...
  // ------------------------------------------------------------
  // SENDER NAME (username under message)
  // ------------------------------------------------------------
  $senderName = function ($m) {
    ...
    return match ($role) {
      'coach' => 'You',
      'admin', 'manager' => ($uName ? '@'.$uName : ucfirst($role)),
      'system' => 'System',
...
<div class="dv-page">
  <div class="card dv-chatCard">
    ...
    <div class="dv-chatHead">
      <div class="dv-headLeft">
        ...
        <div class="dv-subline">
          ...
          {{-- handled by / queue / review --}}
          @if($agentName)
            <span class="dv-agent text-capitalize">
      ...
      <a class="btn btn--ghost back" href="{{ route('coach.disputes.index') }}">Back</a>
    </div>
    ...
    @if($canChat)
      <form class="dv-send"
            method="POST"
            action="{{ route('coach.disputes.message', $dispute) }}"
    ...
    @else
      <div class="dv-closed text-capitalize">
        @if($isFinal)
