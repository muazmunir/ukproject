...
<div class="tx-page">
    ...
    <div class="tx-panel">
        <div class="tx-panel__head">
            <h2>Filters</h2>
            <p>Refine transactions by date range, provider, method, status, and funding type.</p>
        </div>

        <form method="GET" action="{{ route('admin.transactions.index') }}" class="tx-filters">
                ...
                <div class="tx-field">
                    ...
                    <select name="provider" id="provider" class="tx-control">
                        ...
                        @foreach(($filterOptions['providers'] ?? []) as $value => $label)
                            <option value="{{ $value }}" @selected(($filters['provider'] ?? '') === $value)>
                                {{ $label }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="tx-field">
                    ...
                    <select name="method" id="method" class="tx-control">
                        ...
                        @foreach(($filterOptions['methods'] ?? []) as $value => $label)
                            <option value="{{ $value }}" @selected(($filters['method'] ?? '') === $value)>
                                {{ $label }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="tx-field">
                    ...
                    <select name="funding_status" id="funding_status" class="tx-control">
                        ...
                        @foreach(($filterOptions['funding_statuses'] ?? []) as $value => $label)
                            <option value="{{ $value }}" @selected(($filters['funding_status'] ?? '') === $value)>
                                {{ $label }}
                            </option>
                        @endforeach
                    </select>
            ...
            <div class="tx-filter-actions">
                ...
                <a href="{{ route('admin.transactions.index') }}" class="tx-btn tx-btn--secondary">Reset</a>
            </div>
        </form>
    </div>
    ...
    <div class="tx-panel">
                    ...
                    <tbody>
                        @foreach($payments as $payment)
                            ...
                            <tr>
                                <td>
                                    <div class="tx-cell-stack">
                                        @if($payment->reservation_id)
                                            <a href="{{ route('admin.bookings.show', $payment->reservation_id) }}" class="tx-link">
                                ...
                                <td>
                                    <div class="tx-cell-stack">
                                        <strong>{{ $payment->service_title_snapshot ?? optional($reservation?->service)->title ?? '—' }}</strong>
                                        <span>{{ $payment->package_name_snapshot ?? '—' }}</span>
                                    </div>
                                </td>
                                ...
                                <td>
                                    <div class="tx-cell-stack">
                                        <strong>
                                            {{ trim((optional($reservation?->client)->first_name ?? '') . ' ' . (optional($reservation?->client)->last_name ?? '')) ?: '—' }}
                                        </strong>
                                ...
                                <td class="text-center">
                                    @if($payment->reservation_id)
                                        <a href="{{ route('admin.transactions.show', $payment->reservation_id) }}" class="tx-btn tx-btn--table">
                                            View
                                        </a>
                                    @else
        ...
        @else
            <div class="tx-empty">
                ...
                <p>Try adjusting your filters, date range, provider, payment method, funding type, or status.</p>
            </div>
        @endif
    </div>
</div>
