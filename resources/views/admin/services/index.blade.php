...
<section class="card">
          ...
          <tr>
            ...
            <td>
              <div class="row-actions compact">
                {{-- Info --}}
              <a href="{{ route('admin.services.show', $s->id) }}" class="btn icon info" title="Details" aria-label="Details">
  <i class="bi bi-info-circle"></i>
              ...
              @if($status !== 'deleted')
  @if($s->admin_disabled)
    <form method="post" action="{{ route('admin.services.enable', $s) }}" class="inline">
      @csrf
  ...
  @else
   <button class="btn icon danger" data-open="#mDisable" data-id="{{ $s->id }}" data-title="{{ $s->title }}" title="Disable" aria-label="Disable">
...
@push('scripts')
<script>
  const baseServiceUrl = "{{ url('admin/services') }}";
