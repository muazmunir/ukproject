...
@php
  ...
  // Manager only sees Admin requests, but keep badge for clarity
  $stateBadge = $req->state==='pending' ? 'bg-warning text-dark' : ($req->state==='approved' ? 'bg-success' : 'bg-secondary');
@endphp
...
<div class="sls-wrap container py-3">
  ...
  <div class="sls-top">
    <div>
      <div class="sls-title">Leave Request #{{ $req->id }}</div>
      <div class="sls-sub text-capitalize">Full request details • Review attachments • Make decision</div>
    </div>

    <div class="sls-actions">
      <a href="{{ route('admin.support_leave.review') }}" class="btn btn-sm btn-outline-dark">
  ...
  <div class="sls-grid mt-3">
    ...
    <div class="sls-card">
      <div class="sls-card-head">
        ...
        <div class="sls-badges">
          ...
          <span class="badge {{ strtolower($agent->role)==='manager' ? 'bg-primary' : 'bg-secondary' }}">
            {{ $roleLabel }}
          </span>
          ...
          @if($req->type)
            <span class="badge bg-dark">{{ ucfirst((string)$req->type) }}</span>
          @endif
        </div>
      </div>
      ...
      <div class="sls-row">
        ...
        <div class="v">
          ...
          <div class="text-muted small">
            User ID: {{ $agent->id }} • TZ: <span class="mono">{{ $agentTz }}</span>
          </div>
        </div>
      </div>
      ...
      <div class="sls-row">
        ...
        <div class="v">
          <div>
            <span class="mono">{{ $startLocal->format('d M Y, H:i') }}</span>
            →
            <span class="mono">{{ $endLocal->format('d M Y, H:i') }}</span>
          </div>
    ...
    <div class="sls-card">
      ...
      @if(($req->files ?? collect())->count())
        <div class="sls-files">
          @foreach($req->files as $f)
            <a class="sls-file"
               href="{{ route('admin.support_leave.review_request_file.download', $f->id) }}">
      ...
      @if($req->state === 'pending')
        <form method="POST" action="{{ route('admin.support_leave.decide', $req->id) }}" class="sls-decision">
          @csrf
