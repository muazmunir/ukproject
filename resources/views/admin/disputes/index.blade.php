  ...
  <div class="ad-card">
    ...
    <div class="ad-head">
      ...
      <div class="ad-head__right">
        <a href="{{ route('admin.disputes.index') }}" class="ad-btn ad-btn--ghost">
...
@endforeach
        ...
        <button type="button"
                ...
                onclick="location='{{ route('admin.disputes.index', array_filter(['q'=>request('q')])) }}'">
          All
        </button>
      </div>

      <form class="ad-search" method="GET" action="{{ route('admin.disputes.index') }}">
        @if(request('status'))
...
<th style="width:140px;" class="text-end">Actions</th>
        ...
        @forelse($rows as $row)
          @php
            ...
            $status = strtolower((string)($row->status ?? 'open')); // open|in_review|resolved|rejected
            ...
            $chatUrl = route('admin.disputes.show', $row->id);

            // Assigned to (either relation or fallback)
  ...
  @php
    ...
    $staffRole = (string)($row->assigned_staff_role ?? '');
  ...
  @if($staffId > 0)
    <div class="ad-taken">
      {{-- <span class="ad-badge ad-badge--solid">Assigned</span> --}}
      <span class="ad-taken__name">
        {{ $staffName ?: ('Staff #'.$staffId) }}
        {{-- @if($staffRole)
          <span class="ad-muted">({{ $staffRole }})</span>
        @endif --}}
      </span>
    </div>
  @else
  ...
  @endif
            ...
            <td class="text-end">
              <div class="ad-actions">
                ...
                {{-- Take (reservation-level take) --}}
              @php
  $me = (int)auth()->id();
  $staffId = (int)($row->assigned_staff_id ?? 0);
@endphp

@if($staffId > 0 && $staffId === $me)
  <span class="ad-badge ad-badge--solid" title="Assigned to you">My</span>
@elseif($staffId > 0)
  <span class="ad-lock" title="Assigned to another staff member">
    <i class="bi bi-lock-fill"></i>
  </span>
@else
  <span class="ad-muted" title="Waiting in queue">—</span>
@endif
              </div>
            </td>
          </tr>
        @empty
          <tr>
            <td colspan="9" class="ad-empty">
              <div class="ad-empty__title">No disputes found</div>
              <div class="ad-empty__sub">Try changing filters or search keywords.</div>
            </td>
          </tr>
        @endforelse
        </tbody>
