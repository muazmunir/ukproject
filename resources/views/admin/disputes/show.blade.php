...
@php
  ...
  // Role
  $myRole    = (string)(auth()->user()->role ?? 'admin');
  ...
  // ------------------------------------------------------------
  // ACTION PERMISSIONS (matches controller rules)
  // ------------------------------------------------------------
  // Messaging + finalize:
  // - manager can act in any dispute (unless finalized)
  // - admin can act only if assigned to them
  $blockedMsgFinalize = $finalized || (!$isManager && !$isMine);

  // Close-to-queue (close conversation -> in_review) must remain assigned-only for BOTH roles
  $blockedClose = $finalized || !$isMine;
  ...
  // Assigned staff label (prefer controller, else relation)
  ...
  <div class="ad-card">
    ...
    <div class="ad-bar">
      <div class="ad-bar__left">
          ...
          <div class="ad-v">
            @if($isAssigned)
              ...
              @else
                ...
                @else
                  <span class="ad-pill warn" style="margin-left:8px;">Locked</span>
                @endif
              @endif
        ...
        {{-- SLA (elapsed minutes + optional due) --}}
        <div class="ad-kv">
      ...
      <div class="ad-bar__right" style="gap:10px; display:flex; align-items:center; flex-wrap:wrap; justify-content:flex-end;">
        ...
        @if(!$finalized)
          <form method="POST"
                action="{{ route('admin.disputes.finalize', $dispute) }}"
                class="ad-inline"
    ...
    <div class="ad-split">
      ...
      <section class="ad-chat">
        ...
        <div class="ad-chat__body">
          @forelse($clientMessages as $m)
            @php
              $mine = in_array((string)($m->sender_role ?? ''), ['admin','manager'], true);
        ...
        @if(!$isManager && !$isMine)
          <div class="ad-chat__empty text-capitalize" style="margin:10px 0;">
            Read-only: you are not assigned to this dispute.
          </div>
        @endif

        <form class="ad-chat__composer"
              method="POST"
              action="{{ route('admin.disputes.message', $dispute) }}">
          @csrf
      ...
      <section class="ad-chat">
        ...
        @if(!$isManager && !$isMine)
          <div class="ad-chat__empty" style="margin:10px 0;">
            Read-only: you are not assigned to this dispute.
          </div>
        @endif
