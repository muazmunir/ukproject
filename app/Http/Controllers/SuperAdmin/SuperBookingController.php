...
class SuperBookingController extends Controller
{
...
'latest_provider_capture_id'    => $refundState['latest_provider_capture_id'] ?? null,
            ...
            // refund-table-driven latest state
...
'provider_capture_id'                 => $refundState['latest_provider_capture_id'] ?? null,
            ...
            // intentionally not derived from reservation refund columns
        ...
        $paymentRows = collect($reservation->payments ?? [])
            ...
            ->map(function ($payment) use ($refundRows) {
                return [
                    ...
                    // original payment metadata only
                    ...
                    // refund table linked count only
        ...
        return view('superadmin.bookings.show', compact(
            'reservation',
            'finance',
            'refundSummary',
            'refundRows',
    ...
    {
        $reservations = $this->filteredReservationCollection($filters);

        $allCount = $reservations->count();

        $cancelledCount = $reservations->whereIn('status', ['cancelled', 'canceled'])->count();
        $clientNoShowCount = $reservations->where('status', 'no_show_client')->count();

        $coachNoShowCount = $reservations->filter(function ($r) {
            return $r->status === 'no_show_coach'
                || (
                    $r->status === 'no_show'
                    && $r->settlement_status === 'refund_pending'
                    && (int) ($r->coach_penalty_minor ?? 0) > 0
                    && $this->reservationRefundTotalMinor($r) > 0
                    && (int) ($r->platform_fee_minor ?? 0) === 0
                );
        })->count();

        $bothNoShowCount = $reservations->filter(function ($r) {
            return $r->status === 'no_show_both'
                || (
                    $r->status === 'no_show'
                    && $r->settlement_status === 'refund_pending'
                    && (int) ($r->coach_penalty_minor ?? 0) > 0
        ...
        $genericNoShowCount = $reservations->filter(function ($r) {
        ...
        $clientDisputesCount = $reservations->sum(fn ($r) => collect($r->disputes ?? [])->where('opened_by_role', 'client')->count());
        $coachDisputesCount = $reservations->sum(fn ($r) => collect($r->disputes ?? [])->where('opened_by_role', 'coach')->count());

        $clientWinsCount = $reservations->sum(function ($r) {
            return collect($r->disputes ?? [])
                ->where('opened_by_role', 'client')
                ->whereIn('decision_action', ['refund_full', 'refund_service'])
                ->count();
        });

        $coachWinsCount = $reservations->sum(function ($r) {
            return collect($r->disputes ?? [])
        ...
        $platformEarnedMinor = (int) $reservations->sum(function ($r) {
            return (int) ($r->platform_earned_minor ?? 0) + (int) ($r->coach_penalty_minor ?? 0);
        });

        $coachTotalEarningMinor = (int) $reservations->sum(
            fn ($r) => $this->reservationCoachTotalEarningMinor($r)
        );

        return [
            'reservations_count'           => $allCount,
            'completed_count'              => $reservations->where('status', 'completed')->count(),
            'paid_count'                   => $reservations->where('payment_status', 'paid')->count(),

            'cancelled_count'              => $cancelledCount,
            'client_no_show_count'         => $clientNoShowCount,
            'coach_no_show_count'          => $coachNoShowCount,
            'both_no_show_count'           => $bothNoShowCount,
            'generic_no_show_count'        => $genericNoShowCount,

            'refund_count'                 => $reservations->filter(fn ($r) => collect($r->refunds ?? [])->isNotEmpty())->count(),
            'refunded_count'               => $reservations->where('settlement_status', 'refunded')->count(),
            'partial_refund_count'         => $reservations->where('settlement_status', 'refunded_partial')->count(),
            'refund_pending_count'         => $reservations->where('settlement_status', 'refund_pending')->count(),

            'client_disputes_count'        => $clientDisputesCount,
            'coach_disputes_count'         => $coachDisputesCount,
            'client_wins_count'            => $clientWinsCount,
            'coach_wins_count'             => $coachWinsCount,

            'subtotal_minor'               => (int) $reservations->sum('subtotal_minor'),
            'fees_minor'                   => (int) $reservations->sum('fees_minor'),
            'total_minor'                  => (int) $reservations->sum('total_minor'),
            'wallet_used_minor'            => (int) $reservations->sum('wallet_platform_credit_used_minor'),
            'payable_minor'                => (int) $reservations->sum('payable_minor'),

            'refund_total_minor'           => (int) $reservations->sum(fn ($r) => $this->reservationRefundTotalMinor($r)),
            'wallet_refunded_minor'        => (int) $reservations->sum(fn ($r) => $this->reservationWalletRefundedMinor($r)),
            'external_refunded_minor'      => (int) $reservations->sum(fn ($r) => $this->reservationExternalRefundedMinor($r)),
            'platform_fee_refunded_minor'  => 0,

            'coach_earned_minor'           => (int) $reservations->sum('coach_earned_minor'),
            'coach_comp_minor'             => (int) $reservations->sum('coach_comp_minor'),
            'coach_penalty_minor'          => (int) $reservations->sum('coach_penalty_minor'),
            'coach_total_earning_minor'    => $coachTotalEarningMinor,
    ...
    private function buildCharts(array $filters): array
    {
        ...
        $bar = [
            ...
            'values' => [
                ...
                $reservations->whereIn('status', ['cancelled', 'canceled'])->count(),

                $reservations->where('status', 'no_show_client')->count(),
                ...
                $reservations->where('settlement_status', 'refunded')->count(),
                $reservations->where('settlement_status', 'refunded_partial')->count(),
                $reservations->filter(fn ($r) => collect($r->disputes ?? [])->isNotEmpty())->count(),
            ],
        ];

        $bucket = match ($filters['range']) {
            'daily'               => 'hour',
        ...
        $lineBuckets = $reservations
            ->groupBy(function ($r) use ($bucket) {
                $date = $this->reservationActivityDate($r);
        ...
        $line = [
            'bucket' => $bucket,
            'labels' => $lineBuckets->keys()->filter()->values()->all(),
            'bookings' => $lineBuckets->map(fn ($g) => $g->count())->filter(fn ($v, $k) => !empty($k))->values()->all(),
            'sales_minor' => $lineBuckets->map(fn ($g) => (int) $g->sum('total_minor'))->filter(fn ($v, $k) => !empty($k))->values()->all(),
            'refund_minor' => $lineBuckets
                ->map(fn ($g) => (int) $g->sum(fn ($r) => $this->reservationRefundTotalMinor($r)))
                ->filter(fn ($v, $k) => !empty($k))
                ->values()
                ->all(),
            'coach_total_earning_minor' => $lineBuckets
                ->map(fn ($g) => (int) $g->sum(fn ($r) => $this->reservationCoachTotalEarningMinor($r)))
                ->filter(fn ($v, $k) => !empty($k))
                ->values()
                ->all(),
        ];
        ...
        $pie = [
            'labels' => ['Wallet Only', 'Mixed', 'External Only'],
            'values' => [
                $reservations->where('funding_status', 'wallet_only')->count(),
                $reservations->where('funding_status', 'mixed')->count(),
                $reservations->where('funding_status', 'external_only')->count(),
            ],
        ];
    ...
    private function reservationWalletRefundedMinor($reservation): int
    {
        return (int) $this->succeededRefundRows($reservation->refunds ?? [])
            ->sum(fn ($refund) => (int) ($refund->refunded_to_wallet_minor ?? 0));
    }

    private function reservationExternalRefundedMinor($reservation): int
    {
        return (int) $this->succeededRefundRows($reservation->refunds ?? [])
            ->sum(fn ($refund) => (int) ($refund->refunded_to_original_minor ?? 0));
    }

    private function reservationPlatformFeeRefundedMinor($reservation): int
    {
        return 0;
    }

    private function reservationCoachTotalEarningMinor($reservation): int
    {
        return (int) (
            (int) ($reservation->coach_earned_minor ?? 0)
            + (int) ($reservation->coach_comp_minor ?? 0)
            - (int) ($reservation->coach_penalty_minor ?? 0)
        );
    }

    private function reservationActivityDate($reservation)
    {
        return $reservation->completed_at
