...
class SuperClientAnalytics extends Controller
{
    public function show(Request $request, Users $client)
    {
        abort_unless(
            auth()->check() && in_array((string) auth()->user()->role, ['admin', 'manager', 'agent','superadmin'], true),
            403
        );
    ...
    ->withQueryString();
        ...
        return view('superadmin.clients.stats', compact(
            'client',
            'currency',
            'periodLabel',
            'range',
