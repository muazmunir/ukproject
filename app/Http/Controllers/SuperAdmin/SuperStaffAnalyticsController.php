...
class SuperStaffAnalyticsController extends Controller
{
    public function index(Request $request)
    {
        ...
        $serviceLeaders = $this->buildServiceLeaders($filters, $filters['leaderboard_limit']);
        ...
        $chartData = [
            ...
            'service_length_chart' => $this->buildStaffBarChart($serviceLeaders, 'service_days', $filters['leaderboard_limit']),
    ...
    ->from('users')
    ->whereIn(DB::raw('LOWER(users.role)'), ['admin', 'manager'])
    ...
    ->get([
        ...
        DB::raw("
            CASE
                WHEN LOWER(users.role) = 'manager' THEN (
                    SELECT staff_teams.id
                    FROM staff_teams
                    WHERE staff_teams.manager_id = users.id
                      AND staff_teams.is_active = 1
                      AND staff_teams.deleted_at IS NULL
                    LIMIT 1
                )
                ELSE st.id
            END as team_id
    ...
    ->map(function ($u) {
        ...
        return view('superadmin.staff_analytics.index', [
            'filters' => $filters,
            'summary' => $summary,
            'chartData' => $chartData,
    ...
    {
        $role = strtolower((string) $request->get('role', 'all'));
        if (!in_array($role, ['all', 'admin', 'manager'], true)) {
            $role = 'all';
        }
    ...
    protected function buildStaffQuery(array $filters): Builder
    {
        ...
        if ($filters['role'] !== 'all') {
            $q->whereRaw('LOWER(users.role) = ?', [$filters['role']]);
        }
        ...
        if (!empty($filters['team_id'])) {
            ...
            $q->where(function ($sub) use ($teamId) {
                $sub->where(function ($managerQ) use ($teamId) {
                    $managerQ->whereRaw('LOWER(users.role) = ?', ['manager'])
                        ->whereExists(function ($sq) use ($teamId) {
                            $sq->selectRaw('1')
                                ->from('staff_teams')
                                ->whereColumn('staff_teams.manager_id', 'users.id')
                                ->where('staff_teams.id', $teamId)
                                ->where('staff_teams.is_active', true)
                                ->whereNull('staff_teams.deleted_at');
                        });
                })->orWhere(function ($agentQ) use ($teamId) {
                    $agentQ->whereRaw('LOWER(users.role) = ?', ['admin'])
                        ->whereExists(function ($sq) use ($teamId) {
                            $sq->selectRaw('1')
                                ->from('staff_team_members')
                                ->whereColumn('staff_team_members.agent_id', 'users.id')
                                ->where('staff_team_members.team_id', $teamId)
                                ->whereNull('staff_team_members.end_at');
                        });
                });
            });
        }
    ...
    protected function transformStaffRow($row, array $teamMap, array $filters = [])
    {
        ...
        $row->details_url = route('superadmin.staff_analytics.index', array_merge(request()->query(), [
            'staff_id' => $row->id,
            'tab' => 'staff',
        ]));
        ...
        return $q->paginate($filters['ratings_per'], ['*'], 'ratings_page')
            ->through(function ($row) {
                ...
                return (object) [
                    ...
                    'details_url' => $rated
                        ? route('superadmin.staff_analytics.index', array_merge(request()->query(), [
                            'staff_id' => $rated->id,
                            'tab' => 'staff',
                        ]))
                        : null,
    ...
    protected function buildServiceLeaders(array $filters, int $limit): Collection
    {
        $staffQuery = $this->buildStaffQuery($filters);

        $rows = (clone $staffQuery)
            ->select([
                'users.id',
                'users.username',
                'users.first_name',
    ...
    protected function buildTeamMap(array $staffIds): array
    {
        ...
        $managerTeams = StaffTeam::query()
            ->whereIn('manager_id', $staffIds)
            ->where('is_active', true)
            ->whereNull('deleted_at')
            ->with('manager')
            ->get();
    ...
    protected function applySort(Builder $query, string $sort): void
    {
        switch ($sort) {
            ...
            case 'service_desc':
                $query->orderBy('service_start_at', 'asc');
                break;

            case 'service_asc':
                $query->orderBy('service_start_at', 'desc');
                break;
    ...
    protected function sortOptions(): array
    {
        return [
            ...
            'service_desc' => 'Longest Service',
            'service_asc' => 'Shortest Service',
            'name_asc' => 'Name A-Z',
        ];
    }
    ...
    {
        return route('superadmin.staff_analytics.index', array_merge(request()->query(), [
            'tab' => 'ratings',
            'stars' => $stars,
        ]));
    }
